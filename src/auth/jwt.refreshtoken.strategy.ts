...
export class JwtRefreshTokenStrategy  extends PassportStrategy(Strategy,"jwt-refreshtoken") {
  ...
  async validate(req,payload: any) {
    const session = await this.authService.sessionFindOneByRefreshToken(req.body.refreshToken)
    if(!session){
    ...
    return { userId: payload.userId, username: payload.username, state: payload.state };
  }
}